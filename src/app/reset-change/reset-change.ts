...
@Component({
  standalone: true,
  selector: 'app-reset-change',
  imports: [CommonModule, ReactiveFormsModule, RouterModule],
  template: `
  <div class="stage" dir="rtl">
    <section class="card" role="form" aria-labelledby="rc-title">
      <header class="head">
        <div class="brand">
          <div class="logo" aria-hidden="true">🛡️</div>
          <div>
            <h2 id="rc-title">تعيين كلمة مرور جديدة</h2>
            <p class="sub">أدخل كلمة المرور الجديدة وأكّدها.</p>
          </div>
        </div>
        <!-- مؤشّر التقدم (3/3) -->
      ...
      <form class="form" [formGroup]="form" (ngSubmit)="submit()" novalidate>
        <div class="field" [class.invalid]="invalid('newPassword')">
          <label for="npw">كلمة المرور الجديدة</label>
          <div class="input-wrap">
            <input
              ...
              [attr.aria-invalid]="invalid('newPassword')"
            />
            <span class="icon">✦</span>
          </div>
          <small class="hint" *ngIf="invalid('newPassword')">٦ محارف على الأقل.</small>
        </div>

        <div class="field" [class.invalid]="mismatch() || invalid('confirmNewPassword')">
          <label for="cpw">تأكيد كلمة المرور</label>
          <div class="input-wrap">
            <input
              id="cpw"
              class="input"
              ...
              [attr.aria-invalid]="mismatch() || invalid('confirmNewPassword')"
            />
            <span class="icon">✓</span>
          </div>
          <small class="hint" *ngIf="mismatch()">كلمتا المرور غير متطابقتين.</small>
        </div>

        <div class="actions">
          <a class="link" [routerLink]="['/reset/code']" [queryParams]="{ id: resetId() }">رجوع لرمز التحقق</a>
          <button class="btn primary" type="submit" [disabled]="form.invalid || loading()">
            <ng-container *ngIf="!loading(); else loadingTpl">تعيين</ng-container>
          </button>
        </div>
  ...
  `,
  styles: [`
  /* ===== موبايل أولاً (نفس ستايل صفحة البريد) ===== */
  .stage{
  ...
  /* الهوية البصرية نفسها (تدرّج رمادي + حد أحمر + فوكس بنفسجي) */
  .input{
  ...
  /* ===== ≥ 480px: الأزرار والروابط بسطر واحد ===== */
...
export class ResetChangeComponent {
  private fb = inject(FormBuilder);
  private api = inject(ResetService);
  private route = inject(ActivatedRoute);
  private router = inject(Router);

  resetId = signal<string>('');  // من query
  code    = signal<string>('');  // من query
  ...
  constructor(){
    ...
    const id = qp.get('id'); const c = qp.get('code');
    if (id) this.resetId.set(id);
    if (c)  this.code.set(c);
  }

  invalid = (k: 'newPassword' | 'confirmNewPassword') =>
    this.form.controls[k].invalid && this.form.controls[k].touched;

  mismatch = () => {
    const a = this.form.value.newPassword, b = this.form.value.confirmNewPassword;
    return !!a && !!b && a !== b;
  };
  ...
  submit(){
    ...
    if (this.mismatch()){ this.ok.set(false); this.msg.set('كلمتا المرور غير متطابقتين.'); return; }
    ...
    this.api.confirmCode({
      ...
      next: res => {
        this.ok.set(true);
        this.msg.set(res.message || 'تم إعادة تعيين كلمة المرور بنجاح.');
        // إعادة توجيه لتسجيل الدخول بعد نجاح العملية
        setTimeout(() => this.router.navigate(['/login']), 1200);
      },
      error: err => {
        this.ok.set(false);
        this.msg.set(err?.error?.message || 'تعذر إتمام العملية.');
      },
